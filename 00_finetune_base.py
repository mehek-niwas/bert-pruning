...
def plot_training_curves(all_log_histories):
    ...
    fig.suptitle("BERT-base Fine-tuning Curves on GLUE Tasks",
    ...
    for ax, (task_name, cfg) in zip(axes, TASK_CONFIG.items()):
        ...
        train_df = df[df.get("loss", pd.Series(dtype=float)).notna()].copy() \
                   if "loss" in df.columns else pd.DataFrame()
        eval_df  = df[df.get("eval_loss", pd.Series(dtype=float)).notna()].copy() \
                   if "eval_loss" in df.columns else pd.DataFrame()

        ax2 = ax.twinx()

        if not train_df.empty and "epoch" in train_df.columns:
            ax.plot(train_df["epoch"], train_df["loss"],
                    color=color, lw=1.4, alpha=0.45, label="Train loss")
        if not eval_df.empty and "epoch" in eval_df.columns:
            ax.plot(eval_df["epoch"], eval_df["eval_loss"],
                    color=color, lw=2.2, ls="--", label="Val loss")
            pk = cfg["primary_key"]
            if pk in eval_df.columns:
                ax2.plot(eval_df["epoch"], eval_df[pk],
...
def plot_base_metrics(final_results):
    ...
    bars = ax.bar(tasks, values, color=colors, width=0.45,
    ...
    ax.set_ylim(0, 1.18)
    ax.set_ylabel("Score")
    ax.set_title("Baseline BERT-base Performance — GLUE Tasks",
...
def plot_entropy_heatmaps(entropy_per_task):
    ...
    for ax, (task_name, entropy) in zip(axes, entropy_per_task.items()):
        ...
        ax.set_title(f"{task_name.upper()} — Mean Attention Entropy",
    ...
    fig.suptitle("Baseline Per-Head Attention Entropy  (higher = more diffuse)",
                 fontsize=12, y=1.01)
...
def plot_entropy_per_layer(entropy_per_task):
    ...
    for task_name, entropy in entropy_per_task.items():
        ax.plot(range(N_LAYERS), entropy.mean(axis=1),
                marker="o", lw=2, ms=6,
    ...
    ax.set_title("Mean Attention Entropy per Layer — Baseline BERT-base",
                 fontweight="bold", fontsize=13)
...
def main():
    ...
    for task_name, cfg in TASK_CONFIG.items():
        ...
        # Baseline entropy
        drop_cols = [c for c in tokenized["train"].column_names
    ...
    pd.DataFrame(rows).to_csv(os.path.join(RESULTS_DIR, "00_base_results.csv"), index=False)

    print(f"\nAll outputs in {FIGURES_DIR}/ and {RESULTS_DIR}/")
    print("Figures:  00_training_curves.png | 00_base_metrics_summary.png |",
